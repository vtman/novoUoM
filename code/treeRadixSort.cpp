...
const int LONG_LENGTH = 50000;
const int SHORT_LENGTH = 1000;
	...
	if (pvBuffer != nullptr) {
		for (int i = 0; i < 3*nThreads; i++) {
			if (pvBuffer[i] != nullptr) {
				free(pvBuffer[i]); pvBuffer[i] = nullptr;
			}
		}
...
int SortTreeData::radixSort(char * vSortBufferMain, char *vSortBuffer0, char * vSortBuffer1, int n) {
	...
	for (int i = 1; i < n; i++) {
		T = *(long long*)(vSortBufferMain + lenD * i);
		if (T < Tmin) Tmin = T;
		if (T > Tmax) Tmax = T;
	}
	...
	for (int i = 0; i < n; i++) {
		T = *(long long*)(vSortBufferMain + lenD * i) - Tmin;
		*(long long*)(vSortBufferMain + lenD * i) = T;
	}
	...
	for (int i = 0; i < 4; i++) {
		...
		n1 = 0;
		for (int k = 0; k < n; k++) {
			if ((( (int)(*(vSortBufferMain + k * lenD + 16)) >> i) & 1) == 0) {
				memcpy(vSortBuffer0 + n0 * lenD, vSortBufferMain + k * lenD, lenD);
				n0++;
			}
			else {
				memcpy(vSortBuffer1 + n1 * lenD, vSortBufferMain + k * lenD, lenD);
				n1++;
			}
		}
		if (n0 == 0 || n1 == 0)continue;
		memcpy(vSortBufferMain, vSortBuffer0, n0 * lenD);
		memcpy(vSortBufferMain + n0 * lenD, vSortBuffer1, n1 * lenD);
	}
	...
	for (int i = 0; i < nbits; i++) {
		...
		n1 = 0;
		for (int k = 0; k < n; k++) {
			if ((((*(Long64_t*)(vSortBufferMain + k * lenD)) >> i) & 1) == 0) {
				memcpy(vSortBuffer0 + n0 * lenD, vSortBufferMain + k * lenD, lenD);
				n0++;
			}
			else {
...
int SortTreeData::processNew() {
	...
	maxRowsInChunk = LONG_LENGTH / lenD;
	nSmallRows = SHORT_LENGTH / lenD;

	pvBuffer = (char**)malloc(sizeof(char*) * 3* nThreads);

	for (int i = 0; i < 3 * nThreads; i++) {
		pvBuffer[i] = nullptr;
		pvBuffer[i] = (char*)malloc(sizeof(char) * lenD * LONG_LENGTH);
		if (pvBuffer[i] == nullptr) {
...
#pragma omp critical
				{
					...
					if (vDone[ib] == 1) {
						...
						nSize = nRows * lenD;
						#ifdef _WIN32
						_fseeki64(fio, vStart[ib] * lenD, SEEK_SET);
						#else
						fseeko64(fio, vStart[ib] * lenD, SEEK_SET);
						#endif
